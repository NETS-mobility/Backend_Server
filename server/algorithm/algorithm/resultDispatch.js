...
const resultDispatch = async (dispatchResult, revData, direction, is3) => {
  ...
  try {
      ...
      if(is3)
      {
        if(direction == 1) time_end = revData.rev_date + " " + revData.old_hos_arr_time; // case 3일 경우, 병원 도착 시간을 기준으로 2개로 분할
        if(direction == 2) time_start = revData.rev_date + " " + revData.old_hos_arr_time; // case 3 후반 배차의 출발 시각은 동행 시작 시간으로 설정
      }
