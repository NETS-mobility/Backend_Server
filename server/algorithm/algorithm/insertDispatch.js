...
const InsertDispatch = async (dispatchResult, revData, direction, is3) => {
  ...
  try {
    ...
    for(let i = 0; i < dispatchResult.length; i++)
    {
      ...
      const edist = dispatchResult[i].expect_move_distance;
      const etime = dispatchResult[i].expect_move_time 
      ...
      const sql = "insert into `car_dispatch`(`reservation_id`,`car_id`,`netsmanager_number`,`car_move_direction_id`,`departure_address`,`destination_address`,`expect_car_pickup_time`,`expect_car_terminate_service_time`,`expect_move_distance`,`expect_move_time`) values (?,?,?,?,?,?,?,?,?,?);"
      await connection.query(sql, [rev_id, car_id, manager_number, dire, adr_start, adr_end, time_start2, time_end2, edist, etime]);
    }
