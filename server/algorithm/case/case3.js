...
const Case3 = async (testData) => {
  ...
  const ResultData = {
    ...
    expect_move_distance: (toHomeEstimatedDist + toHosEstimatedDist) / (2 * 1000), //GetEstimatedTime돌렸을 때 나온 총 거리
    expect_move_time: (toHomeEstimatedTime + toHosEstimatedTime) / (2 * 60000), //estimatedTime
  };
