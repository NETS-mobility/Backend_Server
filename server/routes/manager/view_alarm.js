...
router.post("/alarmList/", async function (req, res, next) {
  ...
  try {
    ...
    const sql =
      "select m.`netsmanager_name`, ma.`alarm_kind`, ma.`alarm_content`, r.`reservation_id`, u.`user_name`, r.`reservation_submit_date`, r.`pickup_address`, r.`expect_pickup_time`, c.`car_number`, r.`gowithmanager_name` " +
      "from manager_alarm as ma " +
      "left join netsmanager as m " +
      "on ma.`netsmanager_number` = m.`netsmanager_number` " +
      "left join reservation as r " +
      "on ma.`netsmanager_number` = r.`netsmanager_number` " +
      "left join user as u " +
