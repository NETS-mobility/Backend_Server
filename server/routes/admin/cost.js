...
router.post("/extra/setting", async function (req, res, next) {
  ...
  try {
    ...
    for (let i = 1; i <= 5; i++) {
      const sqlc =
        "update `extra_cost` set `extra_cost_unit_value`=?, `extra_cost_per_unit_value`=? where `extra_cost_kind_id`=?;";
      const sqlrc = await connection.query(sqlc, [
    ...
    for (let i = 6; i <= 7; i++) {
      const sqlc =
        "update `extra_cost` set `extra_cost_per_unit_value`=? where `extra_cost_kind_id`=?;";
      const sqlrc = await connection.query(sqlc, [
        extra_cost[i - 1].extra_cost_per_unit_value,
        i,
      ]);
    ...
    const sqlc2 =
      "update `company_time_schedule` set `company_time_start`=?, `company_time_end`=? where `company_time_kind_id`=1;";
    ...
    for (let i = 1; i <= 7; i++) {
      const sqlc =
        "update `manager_extra_pay` set `extra_pay_percentage`=? where `extra_pay_id`=?;";
      const sqlrc = await connection.query(sqlc, [
    ...
    const sqlc3 =
      "update `manager_extra_pay` set `extra_pay_start_time`=?, `extra_pay_end_time`=? where `extra_pay_id`=2;";
    ...
    const sqlc4 =
      "update `manager_extra_pay` set `extra_pay_day_standard_time`=?, `extra_pay_week_standard_time`=? where `extra_pay_id`=1;";
...
router.post("/service/extratime/setting", async function (req, res, next) {
  ...
  try {
    ...
    for (let i = 1; i <= 5; i++) {
      const sqlc =
        "update `service_extratime` set `extratime_data`=? where `extratime_id`=?;";
      const sqlrc = await connection.query(sqlc, [
        data[i - 1].extratime_data,
        i,
      ]);
      if (sqlrc[0].affectedRows == 0) throw (err = 0);
    }
