...
router.post("", async function (req, res, next) {
  ...
  try {
    const sql =
      ...
      "or exists(select * from `payment` as P where `payment_state_id`=1 and R.`reservation_id`=P.`reservation_id`)) " +
    ...
    {
      const sql2 = "select * from `payment` where `payment_state_id`=1 and `payment_type`=1 and `reservation_id`=?;";
      const sqlr2 = await connection.query(sql2, [data1[i].id]);
      const isNeedBasePay = sqlr2[0].length > 0;

      const sql3 = "select * from `payment` where `payment_state_id`=1 and `payment_type`=2 and`reservation_id`=?;";
