...
router.post('', async function (req, res, next) {
    ...
    try {
        ...
        if (serviceKindId == 3 || serviceKindId == 5) // 왕복일 경우 편도 기준으로 바꿈
        {
        ...
        if (overMoveDistance > 0)
        {
            ...
            if (overMoveDistance % moveDistanceUnit != 0)
                movelevel += 1;
        
            overMoveDistanceCost = movelevel * moveDistanceUnitValue;
            if (serviceKindId == 3 || serviceKindId == 5) // 왕복일 경우 2배
        ...
        if (serviceKindId != 1) // 네츠 무브는 제외
        {
            ...
            if (overGowithTime > 0)
            {
                ...
                if (overGowithTime % gowithTimeUnit != 0)
                    timelevel += 1;
        ...
        if (nightmin != 0)
        {
            nightCost = parseInt(TotalBaseCost*(nightmin/totalmin)*(nightRatio-1));
        }
