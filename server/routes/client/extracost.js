...
router.post('', async function (req, res, next) {
    ...
    try {
        ...
        if (serviceKindId == 3 || serviceKindId == 5) // 왕복일 경우
        {
        ...
        // 승차 지연인지 판단
        if (realPickupTime > expPickupTime) // 픽업 시간이 지연되면
        {
        ...
        if (serviceKindId == 3 || serviceKindId == 5) // 왕복일 경우
        {
            if (realHospitalDepartureTime > hopeHospitalDepartureTime) // 병원에서 귀가 출발 시간이 지연되면
            {
        ...
        if (delayTime > 0)
        {
            if (delayTime > (delayTimeMaxUnit-delayTimeFreeUnit)) // 최대 초과 승차 지연 대기 시간으로 설정
                delayTime = delayTimeMaxUnit-delayTimeFreeUnit;
            
            timelevel2 = parseInt(delayTime / delayTimeUnit); // 단계 계산
            if (delayTime % delayTimeUnit != 0)
                timelevel2 += 1;
        ...
        res.status(200).send({ success: true, extraCost: TotalExtraCost });
    }
    catch (err) {
